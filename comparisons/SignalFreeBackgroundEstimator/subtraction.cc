// Test of subtraction performance of rho estimation with the new method (SignalFreeBackgroundEstimator).
...
#include "fastjet/contrib/SignalFreeBackgroundEstimator.hh"
...
int main (int argc, char ** argv) {
  ...
  fastjet::PseudoJet jet_zeroEta(5,0,0,10);

  string included_subs = cmdline.value<string>("-methods", "unsub,area,jetByJetCS_GMBE,jetByJetCS_SFBE_ICS,jetByJetCS_SFBE_puppi,ICS_GMBE,ICS_SFBE_ICS,ICS_SFBE_puppi,puppi");  // use names which are not contained in each other (to avoid problems with the plotting macro)
  included_subs.insert(0,1,',');
  included_subs.push_back(',');

  string input_hard=cmdline.value<string>("-hard");
  ...
  // select jets in certain pT range with rap cut:
  //  Selector sel_jets = SelectorAbsRapMax(jet_rapmax)*SelectorPtMin(jet_ptmin)*SelectorNHardest(2);
  Selector sel_jets = SelectorAbsRapMax(jet_rapmax)*SelectorPtMin(jet_ptmin)*SelectorPtMax(50);
  ...
  //......................................................................
  // [area] [eventWideCS] [ICS_GMBE] area--median subtraction and CS and ICS
  double grid_size = 0.6; // different grid size with respect to /comparisons/review/subtraction-tests.cc, version 1.0.0
  GridMedianBackgroundEstimator * gmbe = new GridMedianBackgroundEstimator(particle_rapmax, grid_size);
  GridMedianBackgroundEstimator * gmbe_neutral = new GridMedianBackgroundEstimator(particle_rapmax, grid_size);
  contrib::SignalFreeBackgroundEstimator *sfbe = new contrib::SignalFreeBackgroundEstimator(particle_rapmax,1.2);
  sfbe->set_window_parameters(0.5,0.4,0.1);;
  contrib::SignalFreeBackgroundEstimator *sfbe_puppi = new contrib::SignalFreeBackgroundEstimator(particle_rapmax,1.2);
  sfbe_puppi->set_window_parameters(0.5,0.4,0.1);
  BackgroundRescalingYPolynomial * rescaling = new BackgroundRescalingYPolynomial(0.9084412, 0, -0.0189777, 0, 4.05981e-05);
  gmbe->set_rescaling_class(rescaling);
  sfbe->set_rescaling_class(rescaling);
  sfbe_puppi->set_rescaling_class(rescaling);
  gmbe_neutral->set_rescaling_class(rescaling);
  ...
  cs.set_background_estimator(gmbe_neutral);
  ...
  vector<double> max_distances={0.2,0.12};  // maximal allowed distances for the individual iterations
  vector<double> alphas={1,1};   // alpha values for the individual iterations
  ics.set_parameters(max_distances,alphas);
  ics.set_ghost_removal(false);  // if set to true, then the unsubtracted ghosts are discarded for the next iteration
  ...
  ics.set_background_estimator(gmbe_neutral);
  ics.set_common_bge_for_rho_and_rhom(true);
  ics.initialize();

  contrib::IterativeConstituentSubtractor ics_SFBE;
  ics_SFBE.set_distance_type(contrib::ConstituentSubtractor::deltaR); // free parameter for the type of distance between particle i and ghost k. There are two options: "deltaR" or "angle" which are defined as deltaR=sqrt((y_i-y_k)^2+(phi_i-phi_k)^2) or Euclidean angle between the momenta
  ics_SFBE.set_parameters(max_distances,alphas);
  ics_SFBE.set_ghost_removal(false);  // if set to true, then the unsubtracted ghosts are discarded for the next iteration
  ics_SFBE.set_ghost_area(0.005); // free parameter for the density of ghosts. The smaller, the better - but also the computation is slower.
  ics_SFBE.set_max_eta(particle_rapmax);   // parameter for the maximal eta to distribute the ghosts. Using the maximal rapidity for selection of input particles here.
  ics_SFBE.set_background_estimator(sfbe);
  ics_SFBE.set_common_bge_for_rho_and_rhom(true);
  ics_SFBE.initialize();

  contrib::IterativeConstituentSubtractor ics_SFBE_puppi;
  ics_SFBE_puppi.set_distance_type(contrib::ConstituentSubtractor::deltaR); // free parameter for the type of distance between particle i and ghost k. There are two options: "deltaR" or "angle" which are defined as deltaR=sqrt((y_i-y_k)^2+(phi_i-phi_k)^2) or Euclidean angle between the momenta
  ics_SFBE_puppi.set_parameters(max_distances,alphas);
  ics_SFBE_puppi.set_ghost_removal(false);  // if set to true, then the unsubtracted ghosts are discarded for the next iteration
  ics_SFBE_puppi.set_ghost_area(0.005); // free parameter for the density of ghosts. The smaller, the better - but also the computation is slower.
  ics_SFBE_puppi.set_max_eta(particle_rapmax);   // parameter for the maximal eta to distribute the ghosts. Using the maximal rapidity for selection of input particles here.
  ics_SFBE_puppi.set_background_estimator(sfbe_puppi);
  ics_SFBE_puppi.set_common_bge_for_rho_and_rhom(true);
  ics_SFBE_puppi.initialize();
  ...
  if (included_subs.find(string(",ICS_GMBE,"))!=string::npos){
    declare_output(output, "ICS_GMBE", jet_ptmin);
  }
  if (included_subs.find(string(",ICS_SFBE_ICS,"))!=string::npos){
    declare_output(output, "ICS_SFBE_ICS", jet_ptmin);
  }
  if (included_subs.find(string(",ICS_SFBE_puppi,"))!=string::npos){
    declare_output(output, "ICS_SFBE_puppi", jet_ptmin);
  }

  
  Subtractor *area_subtractor = new Subtractor(gmbe_neutral);
  ...
  if (included_subs.find(string(",area,"))!=string::npos){
    ...
    output.header << "#   grid size for GMBE        = " << grid_size << endl;
  ...
  //......................................................................
  //   [jetByJetCS_GMBE] ConstituentSubtractor [external]
  if (included_subs.find(string(",jetByJetCS_GMBE,"))!=string::npos){
    double csub_alpha =  0.0;
    double csub_maxDR = -1.0;
    transformers.push_back(TransformerWithName(new contrib::ConstituentSubtractor(gmbe_neutral, 0, csub_alpha, csub_maxDR), "jetByJetCS_GMBE"));
    ...
    declare_output(output, "jetByJetCS_GMBE", jet_ptmin);
  }


  //......................................................................
  //   [jetByJetCS] ConstituentSubtractor using SignalFreeBackgroundEstimator (finding signal seeds with ICS)
  if (included_subs.find(string(",jetByJetCS_SFBE_ICS,"))!=string::npos){
    double csub_alpha =  0.0;
    double csub_maxDR = -1.0;
    transformers.push_back(TransformerWithName(new contrib::ConstituentSubtractor(sfbe, 0, csub_alpha, csub_maxDR), "jetByJetCS_SFBE_ICS"));
    output.header << "# Parameters for the JetByJetCSuentSubtractor_SFBE" << endl;
    output.header << "#   rapidity rescaling for rho" << endl;
    output.header << "#   grid size for SFBE        = " << grid_size << endl;
    output.header << "#   alpha                      = " << csub_alpha << endl;
    output.header << "#   maxDeltaR                  = " << csub_maxDR << endl;
    output.header << "#   description                = " << transformers.back().description() << endl;

    declare_output(output, "jetByJetCS_SFBE_ICS", jet_ptmin);
  }
  
  //......................................................................
  //   [jetByJetCS] ConstituentSubtractor using SignalFreeBackgroundEstimator (finding signal seeds with puppi)
  if (included_subs.find(string(",jetByJetCS_SFBE_puppi,"))!=string::npos){
    double csub_alpha =  0.0;
    double csub_maxDR = -1.0;
    transformers.push_back(TransformerWithName(new contrib::ConstituentSubtractor(sfbe_puppi, 0, csub_alpha, csub_maxDR), "jetByJetCS_SFBE_puppi"));
    output.header << "# Parameters for the JetByJetCSuentSubtractor_SFBE" << endl;
    output.header << "#   rapidity rescaling for rho" << endl;
    output.header << "#   grid size for SFBE        = " << grid_size << endl;
    output.header << "#   alpha                      = " << csub_alpha << endl;
    ...
    declare_output(output, "jetByJetCS_SFBE_puppi", jet_ptmin);
  }
  ...
  while ( mixer.next_event() && iev < nev ) {
    ...
    vector<PseudoJet> hard_jets = sorted_by_pt(cs_hard.inclusive_jets(20.));

    // To test the SignalFreeBackgroundEstimator, selecting mainly events with high jet multiplicity for WW and Z':
    if (hard_jets.size()<4 && (input_hard.find("WW") != std::string::npos)) continue;
    if (hard_jets.size()<3 && (input_hard.find("Zprime") != std::string::npos)) continue;

    hard_jets = sel_jets(hard_jets);
    ...
    gmbe->set_particles(full_event);
    gmbe_neutral->set_particles(neutral_event);

    vector<PseudoJet> subtracted_ics=ics.subtract_event(neutral_event);
    subtracted_ics.insert(subtracted_ics.end(), charged_hard_event.begin(), charged_hard_event.end());
    sfbe->set_particles(neutral_event, subtracted_ics, gmbe_neutral->rho(jet_zeroEta), charged_hard_event);

    // PUPPI: rescale up chg-PU
    ...
    sfbe_puppi->set_particles(neutral_event, puppi_event, gmbe_neutral->rho(jet_zeroEta), charged_hard_event);
    ...
    // ICS_GMBE whole event
    if ((included_subs.find(string(",ICS_GMBE,"))!=string::npos) && is_chs){
      vector<PseudoJet> subtracted_jets = jet_def(subtracted_ics);
      record("ICS_GMBE", hard_jets, subtracted_jets, matching, output, jet_rapmax, iev <= maxprintout);
    }

    // ICS whole event with SignalFreeBackgroundEstimator (using ICS for getting the signal seeds)
    if ((included_subs.find(string(",ICS_SFBE_ICS,"))!=string::npos) && is_chs){
      vector<PseudoJet> subtracted=ics_SFBE.subtract_event(neutral_event);
      subtracted.insert(subtracted.end(), charged_hard_event.begin(), charged_hard_event.end());
      vector<PseudoJet> subtracted_jets = jet_def(subtracted);
      record("ICS_SFBE_ICS", hard_jets, subtracted_jets, matching, output, jet_rapmax, iev <= maxprintout);
    }

    // ICS whole event with SignalFreeBackgroundEstimator (using puppi for getting the signal seeds)
    if ((included_subs.find(string(",ICS_SFBE_puppi,"))!=string::npos) && is_chs){
      vector<PseudoJet> subtracted=ics_SFBE_puppi.subtract_event(neutral_event);
      subtracted.insert(subtracted.end(), charged_hard_event.begin(), charged_hard_event.end());
      vector<PseudoJet> subtracted_jets = jet_def(subtracted);
      record("ICS_SFBE_puppi", hard_jets, subtracted_jets, matching, output, jet_rapmax, iev <= maxprintout);
    }
    ...
    if (iev % iev_periodic == 0){
       if (iev == 3*iev_periodic) iev_periodic*=10;
       output.write(iev);
    }
